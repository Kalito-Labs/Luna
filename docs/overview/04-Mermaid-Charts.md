...
TEXT provider_name "Provider name text"
...
B[idx_appointments_patient_date]

  

B --> B1[patient_id, appointment_date ASC]

  

B1 --> B2[Upcoming appointments]

  

C[idx_appointments_status_date]

  

C --> C1[status, appointment_date ASC]

  

C1 --> C2[Scheduled appointments]
...
D1 --> D2[Patient medications]

  

E[idx_medications_active]
...
E1 --> E2[Active medications only]

  

F[idx_medication_logs_patient_scheduled]

  

F --> F1[patient_id, scheduled_time DESC]

  

F1 --> F2[Medication adherence]
...
E[Delete Patient] -->|CASCADE| F[Delete Appointments]

  

E -->|CASCADE| G[Delete Medications]

  

E -->|CASCADE| H[Delete Medication Logs]

  

E -->|CASCADE| I[Delete Vitals]

  

J[Delete Medication] -->|CASCADE| K[Delete Medication Logs]

  

L[Delete Healthcare Provider] -.->|NULL| M[Appointments provider_id = NULL]
...
S->>V: Get recent vitals

  

V-->>S: Weight, glucose readings
...
- **Eldercare Tables**: 6 (patients, appointments, medications, medication_logs, vitals, healthcare_providers)
...
2. **Patients** → Appointments (1:N with cascade)
3. **Patients** → Medications (1:N with cascade)
4. **Patients** → Vitals (1:N with cascade)
...
9. **Sessions** → Conversation Summaries (1:N with cascade)
10. **Patients** → Medication Logs (1:N with cascade)
